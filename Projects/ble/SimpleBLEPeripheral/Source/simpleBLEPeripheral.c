...
static void simpleProfileChangeCB( uint8 paramID )
{
  ...
  switch( paramID )
  {
    ...
    case SIMPLEPROFILE_CHAR5:
      {      
        uint8 newStr[SIMPLEPROFILE_CHAR5_LEN];
        SimpleProfile_GetParameter(SIMPLEPROFILE_CHAR5, &newStr );
        #if (defined HAL_LCD) && (HAL_LCD == TRUE)
...
static void NpiSerialCallback( uint8 port, uint8 events )
{
  ...
  if (events & HAL_UART_RX_TIMEOUT)   //串口有数据
  {
    ...
    if(numBytes)
    {
      ...
      buf[0] = 0x00;
      buf[4] = 0xff;
      SimpleProfile_SetParameter( SIMPLEPROFILE_CHAR4, SIMPLEPROFILE_CHAR4_LEN, buf );
      //把串口接收到的数据再打印出来
